...
export type ChipAssistProps = Pick<ChipProps, 'disabled' | 'label' | 'leadingIcon' | 'onClick'>;
...
export function ChipAssist({ disabled, label, leadingIcon, onClick }: ChipAssistProps) {
    return (
        <Chip data-bspk="chip-assist" disabled={disabled} label={label} leadingIcon={leadingIcon} onClick={onClick} />
    );
}
